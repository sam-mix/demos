...
const (
	KEY string = "123456"  // 秘钥
	IV  string = "abcdefg" // 向量
	...
	for {
		...
		if n < msg.CLIMSGHEADLEN {
			...
			if uint32(n) != head.MsgLen+msg.CLIMSGHEADLEN {
				...
				txtInputBuffer, err = cryp.AesDecrypt(txtInputBuffer, KEY, IV)
				if err != nil {
					log.Fatal(err)
				}
				if head.MsgID == uint16(pb.MSG_ID_HEART) {
					c2sPb := &pb.C2SHeart{}
					err = proto.Unmarshal([]byte(txtInputBuffer), c2sPb)
					if err != nil {
						log.Fatal(err)
					}
		...
		txtInputBuffer, _ = cryp.AesEncrypt(txtInputBuffer, KEY, IV)
		// tcp server 服务端代码
