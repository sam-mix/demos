...
const (
	topic = "kafka_demo"
	group = "kafka_demo"
	host  = "localhost:9095,localhost:9096,localhost:9098"
)
...
func main() {
	...
	// sarama.Logger = log.New(os.Stdout, "[SARAMA] ", log.LstdFlags) // 可以使用自定义日志存储，全局
	wg := sync.WaitGroup{}
	wg.Add(2)
	producer(&wg) // 生产者
	consumer(&wg) // 消费者
...
func producer(wg *sync.WaitGroup) {
	go func() {
		...
		for i := 1; i < 1000; i++ {
			...
			// buffer.WriteString(fmt.Sprintf("curent: %v", time.Now().Format(time.RFC3339)))
			buffer.WriteString(fmt.Sprintf("curent: %v", i))
